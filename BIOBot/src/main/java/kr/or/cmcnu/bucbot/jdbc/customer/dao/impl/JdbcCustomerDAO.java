package kr.or.cmcnu.bucbot.jdbc.customer.dao.impl;
...
import kr.or.cmcnu.bucbot.jdbc.common.DateUtil;
import kr.or.cmcnu.bucbot.jdbc.customer.dao.CustomerDAO;
import kr.or.cmcnu.bucbot.jdbc.customer.model.Customer;
import kr.or.cmcnu.bucbot.jdbc.customer.model.CustomerRowMapper;
import kr.or.cmcnu.bucbot.service.TeamupService;
...
public class JdbcCustomerDAO extends JdbcDaoSupport implements CustomerDAO {
	...
	public List<String> getPhoneNumber(String phonenumber) {
		String sql = "SELECT CONCAT(phone, ' - ', department, ' ', part) FROM bucheon.phone_inside "
					...
					+ "OR part LIKE '%"+phonenumber+"%' "
					+ "OR (INSTR(part, '~') > 0 "
			        + "    AND department IN ('병동', '호스피스완화의료팀') "
			        + "    AND "+phonenumber+" BETWEEN CAST(SUBSTRING_INDEX(part, ' ', -1) AS UNSIGNED) AND CAST(SUBSTR(part, INSTR(part, '~') + 1) AS UNSIGNED))";
